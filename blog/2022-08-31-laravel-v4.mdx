...
Breaking changes:
- Sorting endpoints in groups by manually editing the endpoint files in `.scribe/` isn't supported anymore. To sort endpoints in groups, use the new `groups.order` key in your config file (described below). It's an easier and more reliable method. Sorting groups by renaming files is still supported, but may be deprecated and removed in a future release.
