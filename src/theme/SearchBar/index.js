...
function useContextualSearchFilters() {
    ...
    function getDocPluginTags(pluginId) {
        ...
        return `docs-${pluginId}-${version.name}`;
    }

    const tags = [
        'default',
        ...Object.keys(allDocsData).map(getDocPluginTags).filter(Boolean),
    ];
...
// Reimplement this so it uses our overridden useContextualSearchFilters() hook
