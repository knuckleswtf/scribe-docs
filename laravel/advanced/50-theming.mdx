...
:::note
The theming system is still experimental. We may change things for v4, if we find ways to make it simpler and more consistent. We're open to [your suggestions](https://github.com/knuckleswtf/scribe/issues)!
:::
...
Then, in the `<body>`, we output the `$index` and `$authentication` information. Those variables hold basic HTML containing the text for the introduction and authentication sections.
